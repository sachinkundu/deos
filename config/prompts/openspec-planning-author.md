...
5. Run `openspec validate <change> --strict` and the repository reading check. Record exact commands and results in `validation.txt`.
6. Do not publish, push, call GitHub or Linear, approve, merge, or run a semantic review. Trusted DEOS code handles those steps after you exit.
7. Write `/deos/output/review-replies.json` as a JSON array. On the first draft, write `[]`. After human feedback, include one `{ "commentId": <root review comment id>, "body": <short exact change or no-change reason> }` entry for every affected root `review_comment`. Do not include issue comments, replies, HTML comments, or resolution requests.
