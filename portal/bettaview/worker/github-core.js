export { locateSelectedText } from "../shared/selection.js";
...
export function parsePullRequestUrl(value) {
  ...
  if (parsed.protocol !== "https:" || parsed.hostname !== "github.com") throw new Error("Use a github.com pull request URL.");
...
export function extractMermaidBlocks(source) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    blocks.push({
      ...
      endLine: end + 1,
      code,
      fingerprint: simpleFingerprint(code),
    });
    index = end;
  }
  return blocks;
...
export function marker(metadata) {
  return `<!-- bettaview:v1 ${base64url(encoder.encode(JSON.stringify(metadata)))} -->`;
}
...
export function readMarker(body = "") {
  ...
  try {
    const value = match[1].replaceAll("-", "+").replaceAll("_", "/");
