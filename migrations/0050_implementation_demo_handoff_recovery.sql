-- Recover a saved demo verdict into its one author repair without another model call.
-- Rebuild the referenced tier table with its parent, preserving all rows and guards.
CREATE TABLE agent_stage_retries_handoff_next (
    ...
    entry_node TEXT CHECK (entry_node IS NULL OR entry_node IN ('implementation_demo_plan','implementation_build')),
...
INSERT INTO agent_stage_retries_handoff_next
SELECT * FROM agent_stage_retries;

CREATE TABLE implementation_retry_tiers_handoff_next (
  retry_id TEXT PRIMARY KEY REFERENCES agent_stage_retries_handoff_next(retry_id),
...
INSERT INTO implementation_retry_tiers_handoff_next SELECT * FROM implementation_retry_tiers;
DROP TRIGGER IF EXISTS sandbox_tier_require_attempt;
DROP TABLE implementation_retry_tiers;
DROP TABLE agent_stage_retries;
ALTER TABLE agent_stage_retries_handoff_next RENAME TO agent_stage_retries;
ALTER TABLE implementation_retry_tiers_handoff_next RENAME TO implementation_retry_tiers;
