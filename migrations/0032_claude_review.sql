-- Protected enrollment metadata contains no token or raw account identity.
CREATE TABLE claude_review_enrollment (
  singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
  secret_version TEXT NOT NULL,
  account_binding TEXT NOT NULL CHECK (length(account_binding) = 64),
  token_hmac TEXT NOT NULL CHECK (length(token_hmac) = 64),
  evidence_reference TEXT NOT NULL,
  enrolled_at TEXT NOT NULL
);
ALTER TABLE orchestration_runs ADD COLUMN independent_review_account_binding TEXT;
CREATE TABLE claude_review_invocations (
  ...
  state TEXT NOT NULL CHECK (state IN ('claimed', 'running', 'finished', 'failed')),
  process_id TEXT,
  safe_cause TEXT CHECK (safe_cause IN ('auth_failure', 'plan_limit', 'review_failure')),
  retry_not_before TEXT,
  cleanup_state TEXT NOT NULL DEFAULT 'pending' CHECK (cleanup_state IN ('pending', 'destroyed', 'failed')),
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL
);
CREATE TABLE claude_review_turns (
  attempt_id TEXT NOT NULL REFERENCES claude_review_invocations(attempt_id),
...
-- Rebuild the existing review graph together to preserve its foreign keys.
CREATE TABLE design_review_rounds_next (
    ...
    outside_provider TEXT NOT NULL CHECK (outside_provider IN ('openrouter', 'claude')),
...
CREATE TABLE design_review_attempts_next (
    ...
    model_provider TEXT NOT NULL CHECK (model_provider IN ('codex', 'openrouter', 'claude')),
    model TEXT NOT NULL,
    ...
    CHECK (
        ...
        OR (phase = 'independent' AND pr_database_id IS NOT NULL AND head_sha IS NOT NULL
            AND model_provider IN ('openrouter', 'claude'))
    ),
    CHECK (
        (accepted = 0)
