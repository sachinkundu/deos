...
export class WorkflowOrchestrator {
  ...
  async run(runId: string, step: WorkflowStepLike): Promise<{ outcome: string; runId: string }> {
    for (;;) {
      ...
      if (instruction.kind === "dispatch_agent") {
        ...
        await step.do(`transition:${instruction.nodeId}:${decision.outcome}`, async () =>
  ...
  private async waitForDefinitionEvent(
    ...
    const consumed = await step.do(`consume-wait:${claimed.delivery_id}`, async () =>
      this.store.consumeWait({
        ...
        transitionId: operationIdentity(
          run.run_id,
          node.id,
          `wait:${decision.outcome}:${claimed.delivery_id}`,
          1,
        ),
        actorId: claimed.actor_id,
        actorType: claimed.actor_type,
  ...
  private async commitOutcome(
    ...
    const transitionId = operationIdentity(
      run.run_id,
      decision.fromNode,
      `transition:${decision.outcome}:${decision.causeReference}:${run.updated_at}`,
      1,
    );
  ...
  private evaluateExecutionOutcome(
    ...
    try {
      ...
      decision = evaluateNodeOutcome(this.definition, nodeId, {
        kind: outcome.kind,
        outcome: "failed",
