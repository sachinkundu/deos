...
export const projectReviewStatus = (
  ...
  if (intent.linear_status === "failed_retryable") actions.push("retry");
  ...
  return {
    ...
    actions,
    ...(error && intent.outcome !== "continued" ? { safeError: error } : {}),
  };
};
...
export class D1ReviewContinuationStore {
  ...
  latestFault(reviewId: string): Promise<{ code: string; message: string } | null> {
    return this.database.prepare(
      `SELECT public_code code,first_provider_message message FROM review_faults
       WHERE review_id=? ORDER BY created_at DESC,fault_id DESC LIMIT 1`,
    ).bind(reviewId).first<{ code: string; message: string }>();
  }
  ...
  async nextLinearAttempt(reviewId: string): Promise<number> {
    const row = await this.database.prepare(
      "SELECT COALESCE(MAX(generation),0) generation FROM review_attempts WHERE review_id=? AND step='linear' AND scope_id='linear-state'",
    ).bind(reviewId).first<{ generation: number }>();
  ...
  async failLinear(reviewId: string, operationId: string, classification: Exclude<LinearMutationClassification, { outcome: "succeeded" }>, now: string): Promise<void> {
    ...
    const faultId = await this.recordFault({ reviewId, operationId, provider: "linear", publicCode: classification.publicCode,
    ...
    const results = await this.database.batch([
      this.database.prepare(`UPDATE review_intents SET linear_status=?,outcome=CASE WHEN ?='host_check_required' THEN 'host_check_required' ELSE outcome END,
        ...
        .bind(status,status,status,now,now,reviewId,operationId),
      this.database.prepare(`UPDATE review_attempts SET outcome=?,fault_id=?,finished_at=?
        WHERE review_id=? AND step='linear' AND scope_id='linear-state' AND finished_at IS NULL`)
        .bind(classification.outcome === "failed" ? "clearly_rejected" : "unclear", faultId, now, reviewId),
    ]);
    if (changes(results[0]) !== 1 || changes(results[1]) !== 1) throw new ReviewContinuationError("linear_fault_conflict");
  }
    ...
    if (step === "github") {
      ...
      const results = await this.database.batch([
        this.database.prepare(
          `UPDATE review_parts SET receipt_status='pending' WHERE review_id=? AND part_id=? AND receipt_status IN ('pending','failed_retryable')`,
        ).bind(reviewId, scopeId),
        this.database.prepare(
          `UPDATE review_intents SET github_status='publishing',updated_at=?
           WHERE review_id=? AND outcome='active' AND github_status IN ('not_started','publishing','failed_retryable')`,
        ).bind(now, reviewId),
      ]);
      if (changes(results[0]) !== 1 || changes(results[1]) !== 1) throw new ReviewContinuationError("part_not_permitted");
    }
  ...
  async failGitHubPart(input: {
    reviewId: string; partId: string; permitId: string; publicCode: string;
    outcome: "clearly_rejected" | "unclear"; error: unknown; now: string;
  }): Promise<void> {
    const faultId = await this.recordFault({
      reviewId: input.reviewId,
      operationId: input.permitId,
      provider: "github",
      publicCode: input.publicCode,
      error: input.error,
      safeFacts: { partId: input.partId, outcome: input.outcome },
      now: input.now,
    });
    const partStatus = input.outcome === "clearly_rejected" ? "failed_retryable" : "host_check_required";
    const results = await this.database.batch([
      this.database.prepare(
        `UPDATE review_parts SET receipt_status=? WHERE review_id=? AND part_id=? AND receipt_status='pending'`,
      ).bind(partStatus, input.reviewId, input.partId),
      this.database.prepare(
        `UPDATE review_attempts SET outcome=?,fault_id=?,finished_at=?
         WHERE review_id=? AND step='github' AND scope_id=? AND permit_id=? AND finished_at IS NULL`,
      ).bind(input.outcome, faultId, input.now, input.reviewId, input.partId, input.permitId),
      this.database.prepare(
        `UPDATE review_intents SET github_status=?,outcome=CASE WHEN ?='host_check_required' THEN 'host_check_required' ELSE outcome END,
         terminal_at=CASE WHEN ?='host_check_required' THEN ? ELSE terminal_at END,updated_at=?
         WHERE review_id=? AND outcome='active' AND github_status='publishing'`,
      ).bind(partStatus, partStatus, partStatus, input.now, input.now, input.reviewId),
    ]);
    if (results.some((result) => changes(result) !== 1)) throw new ReviewContinuationError("github_fault_conflict");
  }
  ...
  async allocateLinear(reviewId: string, operationId: string, now: string): Promise<ReviewIntentRecord> {
    const intentBefore = await this.find(reviewId);
    if (!intentBefore) throw new ReviewContinuationError("d1_read_failed");
    const requestDigest = await sha256Hex(canonicalJson({
      reviewId, issueId: intentBefore.issue_id, stateId: intentBefore.target_state_id, operationId,
    }));
    await this.nextPermit(reviewId, "linear", "linear-state", requestDigest, now);
    const results = await this.database.batch([
      this.database.prepare(
        `UPDATE review_continuation_leases SET phase='linear_pending'
         WHERE review_id=? AND phase IN ('publishing','linear_pending') AND released_at IS NULL`,
      ).bind(reviewId),
      this.database.prepare(
  ...
  async awaitLinearDelivery(reviewId: string, operationId: string, now: Date): Promise<void> {
    ...
    const results = await this.database.batch([
      ...
      ).bind(deadline, now.toISOString(), reviewId, operationId),
      this.database.prepare(
        `UPDATE review_attempts SET outcome='succeeded',finished_at=?
         WHERE review_id=? AND step='linear' AND scope_id='linear-state' AND finished_at IS NULL`,
      ).bind(now.toISOString(), reviewId),
    ]);
    if (changes(results[0]) !== 1 || changes(results[1]) !== 1) throw new ReviewContinuationError("linear_receipt_conflict");
  }

  async prepareLinearRetry(reviewId: string, now: string): Promise<void> {
    const result = await this.database.prepare(
      `UPDATE review_intents SET linear_status='not_started',linear_delivery_deadline=NULL,updated_at=?
       WHERE review_id=? AND outcome='active' AND github_status='done' AND linear_status='failed_retryable'`,
    ).bind(now, reviewId).run();
    if (changes(result) !== 1) throw new ReviewContinuationError("retry_not_allowed");
  }
...
export class ReviewContinuationService {
  ...
  async status(reviewId: string): Promise<ReviewStatusView> {
    const normalized = normalizeId(reviewId);
    const intent = await this.store.find(normalized);
    if (!intent) throw new ReviewContinuationError("review_not_found");
    return projectReviewStatus(intent, await this.store.latestFault(normalized));
  }
}
...
export class ReviewContinuationRpc {
  ...
  async failGitHubPart(assertion: ServiceAssertion, input: {
    reviewId: string; partId: string; permitId: string; publicCode: string;
    outcome: "clearly_rejected" | "unclear"; providerMessage: string;
  }): Promise<ReviewStatusView> {
    await this.authorizeReview("failGitHubPart", assertion, input);
    if (!/^[A-Za-z0-9_.:-]{1,100}$/.test(input.publicCode) || !input.providerMessage) {
      throw new ReviewContinuationError("invalid_input");
    }
    await this.store.failGitHubPart({
      ...input,
      error: new Error(input.providerMessage),
  ...
  async markGitHubReady(assertion: ServiceAssertion, input: {
    ...
    await this.dispatchReviewReady(target, input.reviewId, await this.store.nextLinearAttempt(input.reviewId));
    return this.status(input.reviewId);
  }

  async retryLinear(assertion: ServiceAssertion, input: { reviewId: string }): Promise<ReviewStatusView> {
    await this.authorizeReview("retryLinear", assertion, input);
    await this.store.prepareLinearRetry(input.reviewId, this.now().toISOString());
    const target = await this.store.workflowTarget(input.reviewId);
    if (!target) throw new ReviewContinuationError("review_ready_target_missing");
    await this.dispatchReviewReady(target, input.reviewId, await this.store.nextLinearAttempt(input.reviewId));
    return this.status(input.reviewId);
