...
export class SandboxAgentController {
  ...
  private prompt(
    ...
    if (job.inputs.includes("implementation_context")) return [job.prompt,
      ...
      'Keep temporary helper requests in /deos/output/requests/, outside the repository. If a restored patch contains .deos-requests scratch files, move them outside the repository before final checks; preserve any useful proof fixture as an intentional repository test. Creating or editing any repository file after a check invalidates its result for the new tree.',
      'For Python tests, python3 -m venv /deos/output/python-env creates an author-owned environment with pip. Install the repository test dependencies there using --cert=/etc/cloudflare/certs/cloudflare-containers-ca.crt; use its Python executable for checks. Do not assume missing global pip or pytest makes the tests unavailable.',
      'Provider fixture calls alone prove the adapter, not the changed app. Run the changed application flow with its test transport bound to the scoped provider adapter; retain proof that GitHub completion precedes Linear continuation through that code path. Required current browser and Showboat proof cannot be replaced by screenshots from an earlier try.',
      'Before returning completed, call deos-implementation with {"action":"status"}. It lists checks matching the current tree, stale checks, and proof kinds. Rerun every required check on that final tree; do not treat earlier passes, a full checklist, or an empty current-check list as success. Missing tools or browser proof must be repaired or reported as needs_human, never waived in assumptions.',
