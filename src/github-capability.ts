...
export class GitHubCapabilityAdapter {
  ...
  async upsertDesignReviewCheck(input: {
    repository: string;
    headSha: string;
    externalId: string;
    detailsUrl: string;
    ...
    const token = await this.tokens.token();
    const name = "DEOS OpenSpec design review";
    const list = await this.json(
      token,
      `/repos/${input.repository}/commits/${input.headSha}/check-runs?check_name=${encodeURIComponent(name)}`,
    ) as { check_runs?: Array<{ id?: number; external_id?: string }> };
    if (!Array.isArray(list.check_runs)) throw new Error("GitHub design Check Run list is invalid");
    const matches = list.check_runs.filter((check) => check.external_id === input.externalId);
    if (matches.length > 1) throw new Error("GitHub design Check Run selection is ambiguous");
    const payload = {
      name,
      head_sha: input.headSha,
      external_id: input.externalId,
    ...
    try {
      const response = await this.json(
        token,
        id === undefined ? `/repos/${input.repository}/check-runs` : `/repos/${input.repository}/check-runs/${id}`,
        { method: id === undefined ? "POST" : "PATCH", body: payload },
      ) as { id?: number };
    ...
    const confirmed = await this.json(token, `/repos/${input.repository}/check-runs/${id}`) as {
      id?: number;
      external_id?: string;
      head_sha?: string;
      details_url?: string;
    ...
    if (
      ...
      confirmed.head_sha !== input.headSha || confirmed.details_url !== input.detailsUrl ||
      confirmed.conclusion !== input.conclusion || typeof confirmed.html_url !== "string"
    ) throw new Error("GitHub design Check Run read-back mismatch");
    return { checkRunId: String(id), url: confirmed.html_url, reconciled };
  }
