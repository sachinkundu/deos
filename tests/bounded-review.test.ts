...
const discover = () => reduceReviewCycle(reduceReviewCycle(initial(), { type: 'invoke', slot: 'discovery', invocationId: 'child1', inputDigest: 'a'.repeat(64) }), { type: 'result', slot: 'discovery', invocationId: 'child1', inputDigest: 'a'.repeat(64), lifecycleVerified: true, result });
...
test('two failed discovery calls require reconciliation; recheck instead keeps all findings open', () => {
    for (const slot of ['discovery', 'recheck']) {
        ...
        if (slot === 'recheck')
            assert.deepEqual(state.openIds, ['F1']);
...
test('source citation matching normalizes URLs without accepting host prefixes', () => {
    const envelope = { searchDisposition: 'sources_used', sources: [{ id: 'S1', title: 'Docs', url: 'https://example.com', claimLocator: 'F1' }] };
    assert.equal(validateSources(envelope, { F1: 'See [Docs](https://example.com).' })[0].url, 'https://example.com/');
    assert.throws(() => validateSources(envelope, { F1: 'See https://example.com.attacker.invalid/' }), /uncited_search_result/);
});
