...
def test_linear_retry_timestamp_window_is_directional() -> None:
    ...
    def signed_headers(at: datetime) -> dict[str, str]:
        return {
            "Linear-Timestamp": str(int(at.timestamp() * 1000)),
            "Linear-Signature": hmac.new(b"secret", body, hashlib.sha256).hexdigest(),
        }

    acl.verify(body, signed_headers(now - timedelta(hours=8, minutes=15)), now)
    acl.verify(body, signed_headers(now + timedelta(minutes=5)), now)
    with pytest.raises(InvalidWebhook, match="stale"):
        acl.verify(body, signed_headers(now - timedelta(hours=8, minutes=15, milliseconds=1)), now)
    with pytest.raises(InvalidWebhook, match="future"):
        acl.verify(body, signed_headers(now + timedelta(minutes=5, milliseconds=1)), now)
