...
test("GitHub planning adapter replaces one scoped manifest on one ready pull request", async () => {
  ...
  const firstFiles = [
    ".openspec.yaml", "proposal.md", "specs/old/spec.md", "design.md", "tasks.md",
...
test("Linear label resolver accepts only the documented bounded GraphQL shape", async () => {
  const queries: string[] = [];
  const adapter = new LinearCapabilityAdapter(
    "https://api.linear.test/graphql",
    "linear-access-token",
    {
      fetch: async (_input, init) => {
        ...
        queries.push(body.query);
        assert.deepEqual(body.variables, { id: "issue-1" });
        return Response.json({
          data: {
            issue: {
              id: "issue-1",
              ...
              labels: {
                nodes: [
                  { id: "label-2", name: "simple-workflow" },
                  { id: "label-1", name: "test" },
                ],
              },
            },
          },
  ...
  const observation = await adapter.readIssueLabels("issue-1");
  assert.deepEqual(observation.labels, ["simple-workflow", "test"]);
  assert.equal(observation.providerDigest.length, 64);
  assert.equal(queries.length, 1);
  assert.match(queries[0], /labels \{ nodes \{ id name \} \}/);
  assert.equal(queries[0].includes("issueUpdate"), false);
});
