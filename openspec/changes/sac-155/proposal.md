...
- Add a clear name on each site so people know which site they use.
- Let both sites use the same D1 and R2 data, with the same GitHub and Linear links.
- Add a chosen release step that moves reviewed work from `main` to the release branch.
- Use GitHub CI to check and deploy each branch for its own site. If a check or deploy fails, keep live on its last good release.
