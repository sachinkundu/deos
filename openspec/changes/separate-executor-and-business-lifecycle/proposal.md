...
- Adding the broader independent telemetry and status projection tracked separately by SAC-103.
...
- `workflow-dispatch`: Preserve resumable run identity and detect premature Workflow completion without allocating a replacement run.

## Impact

- Affects the versioned workflow definition, evaluator and Workflow orchestration contract, D1 run and transition records, event inbox handling, and stable Linear operator notices.
