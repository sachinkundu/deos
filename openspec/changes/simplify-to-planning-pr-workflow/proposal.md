...
The current DEOS definition spans the full delivery lifecycle, which makes a first live run difficult to observe and improve one step at a time. A smaller planning-only workflow will provide a concrete, independently provable foundation by producing the complete OpenSpec planning bundle before implementation begins.
...
- Run one OpenSpec planning agent that creates or updates one human-readable GitHub pull request containing the proposal, specifications, design, and tasks while DEOS owns every Linear transition.
...
- Run an implementation agent or execute the generated tasks.
...
- Establishes one complete planning-bundle traceability boundary from the triggering issue through the verified merge.
