...
The current DEOS definition spans the full delivery lifecycle, which makes a first live run difficult to observe and improve one step at a time. A smaller workflow that stops after proposal and specification planning will provide a concrete, independently provable foundation before design, task generation, and implementation are added.
...
- Generate a design, implementation tasks, or runtime changes, or run an implementation agent.
...
- `workflow-dispatch`: Selects and freezes the simplified definition for a new issue run only when its selector is enabled and the accepted `Todo` transition has the `simple-workflow` Linear label; otherwise selects and freezes the existing full definition.
