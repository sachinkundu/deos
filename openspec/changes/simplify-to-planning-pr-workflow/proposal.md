...
- Add an inactive simplified workflow definition that starts when a person moves a configured Linear issue to `Todo`.
...
- `simplified-planning-workflow`: Defines the planning-only graph, its human decisions, its workflow-owned merge, and the proof required before the planning slice succeeds.

### Modified Capabilities

None. The existing dispatch, workflow-state, provider-capability, and Sandbox contracts remain the platform boundaries used by the new definition.

## Impact

- Adds a separately selectable workflow definition and its external agent prompt bundle without replacing the current production definition.
