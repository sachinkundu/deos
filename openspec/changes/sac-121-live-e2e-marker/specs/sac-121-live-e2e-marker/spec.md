...
### Requirement: One final GitHub work product per agent attempt
Each agent attempt SHALL publish exactly one final GitHub work product.

#### Scenario: Agent attempt completes
- **WHEN** an agent attempt reaches its final publishing step
- **THEN** exactly one GitHub work product is published for that attempt
