...
- [ ] 3.1 Add the fixed Claude profile to a new immutable definition and freeze its account binding for each new run. Preserve existing OpenRouter runs and legacy route settings.
- [ ] 3.2 Reuse existing semantic validators and hash-checked proof publication. Require provider proof and cleanup before accepting a review.
- [ ] 3.3 Show fixed Claude settings for the new definition and safe failure causes. Preserve prior-definition rollback and its required OpenRouter model.
