## 1. Durable Read Model

- [x] 1.1 Add the issue index, visit linkage, and governed work-link schema with additive migrations and migration coverage.
- [x] 1.2 Preserve safe Linear issue identity and exact visit sequence in authenticated ingress and orchestration writes.
- [x] 1.3 Add deterministic historical backfill and inventory tooling that never guesses ambiguous issue, attempt, wait, or work-link relationships.

## 2. Safe Portal Projection

- [x] 2.1 Inventory stored workflow definitions and add exact-digest presentation manifests with complete node and edge coverage.
- [x] 2.2 Implement the closed read-only D1 store, visit reconstruction, cycle projection, safe waits, attempts, and governed links.
- [x] 2.3 Implement the explicit response DTO, validation, redaction, safe error contract, run list, and selected-run routes.

## 3. Authentication And Deployment Boundary
...
- [x] 6.4 Run real Linear provider wait-and-resume and terminal canaries, capture matching Linear, D1, portal, and timing proof, and disable canary dispatch afterward.
