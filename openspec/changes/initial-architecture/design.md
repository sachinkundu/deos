...
The design provides a small Python-first boundary for authenticated ingress and domain logic, with durable dispatch isolated behind a provider-supported Queue-consumer adapter. Deterministic fakes remain available for fast tests, while a Cloudflare deployment and a test Linear project exercise the actual integration path.
...
- [Cloudflare runtime mismatch] → keep domain logic platform-neutral, isolate binding adapters, and require provider-originated evidence for the selected runtime.
