...
- [ ] Add OpenTelemetry-compatible correlation across ingress, queue, transitions, and external calls. Stage plan: `add-otel-workflow-correlation`; Linear: SAC-80.
...
- [ ] Add R2 artifact provenance records and deterministic evidence capture for workflow runs. Stage plan: `add-r2-artifact-provenance`; Linear: SAC-79.
- [ ] Migrate the Python HTTP ingress Worker to TypeScript, update the ingress tests and project tooling accordingly, and require the full test suite, lint/type checks, Wrangler dry runs, and strict OpenSpec validation to pass before completion. Stage plan: `migrate-ingress-worker-to-typescript`; Linear: SAC-78.
