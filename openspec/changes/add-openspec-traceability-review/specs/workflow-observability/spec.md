...
Each trace event SHALL use the flow's shared link ID. It SHALL name the review input ID, review, run, job, try, stage, mode, and round. It SHALL also name a safe plan ID and the reviewed head, when one exists. It SHALL name the base finding set and each related finding ID. It SHALL name the model provider, model, thought setting, tool version, result, file list, and provider task. It SHALL say whether the result was started, reused, made stale, rejected as inconsistent, or sent to human judgment. These fields SHALL link events to saved review records. They MUST NOT expose plan text.
...
- **WHEN** one review stage contradicts its own fixed rating without a matching source change
- **THEN** an event names both safe review IDs and the human-judgment decision without exposing finding text
