...
- Open one planning pull request after the self-check passes or uses the shared repair limit. Run the independent review through the same Codex coding-agent harness, but configure that harness with an OpenRouter model chosen in the DEOS settings page. Save the harness, provider, and model with the run so a settings change cannot alter active work. Repair the same pull request. After each repair, let the Codex self-check inspect the exact new head. Then let the Codex harness using the OpenRouter model recheck every finding in its base set before human review.
