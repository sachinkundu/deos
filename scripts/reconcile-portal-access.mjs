...
let provider = providers.find((candidate) => candidate.type === "onetimepin");
if (!provider) {
...
if (!application) {
  application = await request("POST", `/accounts/${accountId}/access/apps`, {
...
if (!policy) {
  policy = await request("POST", `/accounts/${accountId}/access/apps/${application.id}/policies`, {
