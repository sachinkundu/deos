...
Durable GitHub and Linear work products go through attempt-scoped capability endpoints. Each request is schema-checked, restricted to the trial repository or issue, and recorded under a stable operation identity. GitHub uses a short-lived App installation token. Linear capabilities can write notes and artifact references but cannot mutate issue state. Workflow-owned transitions use the Linear app actor and are confirmed only by ordered signed-delivery evidence.

Cleanup has two views: a Worker cron reconciles D1-known attempts without waking a missing process, and a repository schedule compares Cloudflare provider inventory to D1. Associated and standalone orphans produce stable Linear cleanup issues through the trusted Worker.
