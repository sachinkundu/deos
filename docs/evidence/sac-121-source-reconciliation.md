...
|---|---|---|
| D1 migration | The lifecycle migration is `0008_explicit_business_lifecycle.sql`, not `0007`, and carries forward `current_visit_sequence` and `last_transition_id`. | Current main already uses `0007_workflow_visit_identity.sql`; renumbering preserves sequential migration order and rebuilding `orchestration_runs` must not discard the newer traversal authority. |
...
| Tests | Lifecycle tests are combined with main's visit, cumulative-patch, and native OpenSpec tests. Migration tests apply `0007` then `0008` and verify visit identity survives the table rebuild; frozen-definition tests cover deployed typed v4 and legacy v10 shapes. | The reconciled assertions cover the combined contract instead of either branch in isolation. |
...
No production migration, Worker deployment, provider trigger, or Linear state
transition is part of SAC-121's source-only reconciliation proof.
