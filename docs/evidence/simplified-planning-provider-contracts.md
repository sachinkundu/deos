...
Refreshed on 2026-08-24 before the selector or provider canary was enabled.
...
- Linear documents `data` as the serialized webhook entity. The current
  official `@linear/sdk` webhook type is `IssueWebhookPayload`: `labelIds` is a
  string array and `labels` is an array of child objects containing `id` and
  `name`. Ingress accepts only matching ids from those two fields. Missing,
  malformed, duplicate, or inconsistent fields become explicit `unavailable`
  evidence; Queue never repairs that evidence with a later GraphQL read.
...
The earlier read-only workspace lookup found no `simple-workflow` label. Creating that
