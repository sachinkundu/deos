...
The [task-recovery Showboat read-back](sac-182-task-recovery.md) confirms the active deployment, container rollout, retry history and running process `5b2ff642-b0e7-4c19-a33f-cefc8bb21139`. The prior attempt is terminal and destroyed in `agent_attempts`; its `implementation_tries` summary still incorrectly says running. That projection needs repair before the final implementation PR.
