...
import { claudeReadCommand } from "/deos/bin/claude-review-read.mjs";
import { readSnapshot } from "/deos/bin/native-review-read.mjs";
...
test('Demo readers open the exact frozen approved, candidate and context inventory', async () => {
  ...
  const read = async(command: string) => {
    ...
    return execute(process.execPath,[resolve('/deos/bin/claude-review-read.mjs'),'--request-file',request],
...
test("Claude reader launches with large file context and preserves command errors", async () => {
  ...
  const script = resolve("/deos/bin/claude-review-read.mjs");
...
test("Claude file reader executes quoted searches over frozen sources without shell expansion", async () => {
  ...
  const read = async (command: string) => {
    await writeFile(request, JSON.stringify({ state, command }));
    return await run(process.execPath, [resolve("/deos/bin/claude-review-read.mjs"), "--request-file", request],
      { env: { ...process.env, DEOS_ERROR_OUTPUT_ROOT: dir } });
  };
  try {
