...
import { recoverCaptures } from '/deos/bin/supervisor-capture.mjs';
...
test('SIGKILL preserves the exact parent transcript for failure collection and replay', async () => {
    ...
    try {
        ...
        await writeFile(script, `import { trustedCapture } from ${JSON.stringify(new URL('/deos/bin/supervisor-capture.mjs', import.meta.url).href)};
