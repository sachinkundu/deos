# Cloud BettaView and complete DEOS review story
...
Deploy BettaView at `bettaview.voxdez.com` as the easier way to read and review a GitHub pull request. Keep every existing human review action. Let the DEOS visualization open either GitHub or BettaView for the same planning pull request. Let an interested reader inspect the complete review-and-repair story from the first author draft through the human-review gate.
...
The cloud split keeps the first three behaviors and replaces the last two. The source references are [BettaView's current server](https://github.com/sachinkundu/bettaview/blob/de22c3cf9a95285647bbf31cac1d6242bef6142a/server/index.js), [GitHub adapter](https://github.com/sachinkundu/bettaview/blob/de22c3cf9a95285647bbf31cac1d6242bef6142a/server/github.js), and [trace rendering](https://github.com/sachinkundu/bettaview/blob/de22c3cf9a95285647bbf31cac1d6242bef6142a/src/traceability-view.js).
...
7. BettaView asks DEOS for the current accepted trace associated with that repository and PR number.
8. If the reviewed head equals the live GitHub head, BettaView shows the trace as current. If not, it shows the existing stale-head warning and never silently rebinds the evidence.
9. The reader can comment, reply, approve, request changes, or annotate a Mermaid diagram. GitHub receives the action under that reader's GitHub user identity.

### Read the complete review story

The DEOS portal adds a **Review story** view for the run. BettaView shows a **View DEOS review story** link after DEOS returns context for the loaded PR. This backlink is discovered after loading; it does not add a run identifier to the BettaView entry URL.
...
flowchart LR
    ...
    Access --> BV[BettaView Worker + static assets]
    BV -->|GitHub App user token| GH[GitHub PRs, threads, reviews, annotation branch]
    BV -->|service binding / RPC<br/>repository + PR number| DEOS[DEOS portal Worker]
    DEOS --> D1[(D1 run and review state)]
    DEOS --> R2[(R2 hash-verified artifacts)]
    DEOS -->|accepted trace + reviewed head + story URL| BV
...
### DEOS review-story read model

Build a new read model instead of weakening the current accepted-trace projection. The story query includes all run visits, agent attempts, review jobs, candidates, head bindings, author dispositions, provider operations, manifests, and cleanup records. It must not filter the story to only `accepted = 1` or reused reviews.
...
### 2. BettaView cloud-compatible core
...
### 3. Access and human GitHub authorization
...
### 4. GitHub review feature parity
...
- Do not deploy as the preferred reader until all current BettaView review actions pass provider-originated checks.

### 5. DEOS trace consumption and dual links
...
- Add **View DEOS review story** inside BettaView after the trace context is resolved.

### 6. Complete DEOS review story
...
### 7. Real end-to-end canary

Run one new Linear-triggered traceability workflow after both deployments. Preserve:

- provider-originated Linear delivery and final Human Review state;
- one planning PR and exact heads before and after any repair;
- actual internal and external comments plus every author disposition;
- a real applied change or a clearly justified no-change response;
- BettaView trace rendering from DEOS, not from a checked-in or locally generated sidecar;
- a BettaView comment, reply, Mermaid annotation, and review decision visible in GitHub under the human tester;
- both DEOS node-detail links and the BettaView backlink to the review story;
...
Stop at the first provider failure, retain the completed prefix, and retry only the failed compatible tail after the defect is fixed.
...
- the DEOS story includes failed and successful attempts plus the actual available review, response, disposition, diff, and recheck records;
...
- both repositories have focused contract tests, production builds, deployed read-back, and provider-originated proof.
...
After this plan is accepted, create coordinated OpenSpec changes in DEOS and BettaView. Keep the two implementations in separate repository pull requests with a versioned integration contract. Do not begin either implementation from this planning document alone.
