...
export function unwrapGroundedReview(envelope){
 if(!envelope||typeof envelope.review!=='object')throw new Error('invalid grounded review envelope');
...
export async function saveGroundedReview(envelope,index){
 ...
 if(prior.some(item=>item.index===index))throw new Error('duplicate grounding turn');
...
export async function checkAuthorSources(check,{cwd,change}){
 ...
 try{
  ...
  const claims={};
  if(!Array.isArray(sidecar.sources))throw new Error('uncited_search_result: sources must be an array');
  for(const source of sidecar.sources){
   const match=typeof source.claimLocator==='string'&&source.claimLocator.match(/^(.+):([1-9][0-9]*)$/);
   if(!match||!match[1].startsWith(`openspec/changes/${change}/`)||match[1].split('/').includes('..'))throw new Error('uncited_search_result: invalid author claim locator');
  ...
  if(!(error instanceof SyntaxError)&&error.code!=='ENOENT'&&!/uncited_search_result|invalid_source_record|invalid source/.test(error.message))throw error;
