...
const citesUrl = (claim, expected) => {
    ...
    return [...claim.matchAll(/https:\/\/[^\s<>"`]+/g)].some(match => {
        ...
        for (;;) {
            try {
                if (new URL(candidate).href === expected)
                    return true;
            }
            catch (error) {
...
export function validateSources(result, claims, captured = []) {
    ...
    return result.sources.map(raw => {
        ...
        try {
            ...
            throw new Error('invalid_source_record: malformed URL', { cause });
        }
...
export function validateDispositions(value, findings) {
    if (!Array.isArray(value))
        throw new Error('invalid_author_response: missing disposition set');
    const ids = new Set(findings.map(item => item.id));
    const seen = new Set();
    ...
    if (seen.size !== ids.size)
        throw new Error('invalid_author_response: incomplete disposition set');
    return result;
}
function validateIndependent(value) {
    record(value, 'independent result');
    if (!Array.isArray(value.findings))
        throw new Error('invalid independent findings');
    const seen = new Set();
    const findings = value.findings.map(item => {
        record(item, 'independent finding');
        if (typeof item.id !== 'string' || !/^[a-zA-Z0-9][a-zA-Z0-9:._-]{0,299}$/.test(item.id) || seen.has(item.id))
            throw new Error('invalid independent finding id');
        seen.add(item.id);
    ...
    if (!Array.isArray(value.sourceEvidence))
        throw new Error('missing independent source evidence');
...
export function createReviewCycle({ runId, phase, attemptId, inputDigest }) {
    if (!['planning', 'design'].includes(phase))
...
export function reduceReviewCycle(prior, event) {
    ...
    if (state.schema !== REVIEW_SCHEMA)
        throw new Error('unsupported review schema');
    const slot = event.slot && state[event.slot];
    if (event.type === 'invoke') {
        if (!['discovery', 'recheck'].includes(event.slot) || state.status !== 'active')
            throw new Error('invalid review invocation');
        if (slot.status === 'accepted' || slot.status === 'unavailable' || slot.invocations.length >= 2 ||
            slot.invocations.some(item => item.status === 'started'))
            throw new Error('review slot is consumed');
        ...
        if (!['discovery', 'recheck'].includes(event.slot))
            throw new Error('invalid result slot');
        const invocation = slot.invocations.at(-1);
        if (!invocation || invocation.id !== event.invocationId || invocation.status !== 'started')
            throw new Error('unobserved_child_invocation');
        if (event.type === 'failure') {
            invocation.status = 'failed';
            invocation.cause = record(event.cause, 'original failure');
            ...
            if (event.inputDigest !== invocation.inputDigest || !event.lifecycleVerified)
                throw new Error('unobserved_child_invocation');
            ...
            if (event.slot === 'discovery') {
                ...
                if (!state.findings.length) {
                    state.recheck.status = 'not_required';
                    state.repair.outcome = 'not_required';
                }
            }
            else
                state.openIds = state.findings.filter(item => slot.result.ratings[item.id] !== 'fixed').map(item => item.id);
        }
    }
        ...
        if (!state.repair.started || state.repair.outcome !== null || state.recheck.invocations.length)
            throw new Error('invalid repair completion');
        state.repair.outcome = event.passed ? 'checked' : 'failed';
        if (event.passed)
            state.currentDigest = checkedDigest(event.candidateDigest);
    }
        ...
        if (state.discovery.status !== 'accepted' || !['accepted', 'unavailable', 'not_required'].includes(state.recheck.status) ||
            event.candidateDigest !== state.currentDigest || state.status !== 'active')
            throw new Error('incomplete review publication');
        ...
        if (!state.currentHead || state.independent.result || event.head !== state.currentHead || state.status !== 'active')
            throw new Error('independent slot mismatch');
        state.independent.result = validateIndependent(event.result);
        state.independent.attemptId = event.attemptId;
        state.reviewedHead = event.head;
        if (!state.independent.result.findings.length)
            state.response.result = [];
    }
    else if (event.type === 'author_response') {
        if (!state.independent.result || state.response.result || state.status !== 'active')
            throw new Error('author response slot mismatch');
        ...
        if (!['independent', 'response'].includes(event.slot) || slot.result || state.status !== 'active')
            throw new Error('invalid result slot');
        slot.invalidCount++;
        slot.causes = [...(slot.causes ?? []), record(event.cause, 'invalid result cause')];
        if (slot.invalidCount >= 2)
            state.status = 'manual_reconciliation_required';
    }
    else if (event.type === 'head_updated') {
        if (!state.independent.result || !state.response.result || state.status !== 'active')
            throw new Error('revision before initial review');
        state.currentHead = text(event.head, 'current head');
    }
    else
        throw new Error('unknown review cycle event');
    return state;
}
export function reviewGateEvidence(state) {
    if (state.status !== 'active' || !state.currentHead || !state.independent.result || !state.response.result)
        throw new Error('incomplete human gate evidence');
...
export async function verifyReviewJournal(journal, { attemptId, runId, inputDigest, candidateFiles, parentTranscript, hash, continuation = null, model = null }) {
    ...
    if (JSON.stringify(candidateFiles) !== JSON.stringify(journal.checkedCandidate.files))
        throw new Error('author_completion_verification_mismatch: final candidate');
    if (!Array.isArray(journal.events) || !Array.isArray(journal.children))
    ...
    if (continuation && (JSON.stringify(journal.events.slice(0, prefixEvents.length)) !== JSON.stringify(prefixEvents) ||
        ...
        JSON.stringify(journal.initialCandidate) !== JSON.stringify(continuation.journal.initialCandidate)))
        throw new Error('review continuation journal prefix changed');
    const parentEvents = parentTranscript.split('\n').filter(line => line.trim()).map(line => JSON.parse(line));
    const childIds = new Set(prefixChildren.map(child => child.invocationId));
    for (const child of journal.children.slice(prefixChildren.length)) {
        const invocation = journal.events.find(event => event.type === 'invoke' && event.invocationId === child.invocationId);
        if (!invocation || childIds.has(child.invocationId) || child.parentAttemptId !== attemptId ||
            child.start?.hook_event_name !== 'SubagentStart' || child.start.agent_id !== child.invocationId ||
            child.start.agent_type !== 'deos_reviewer' || (model && child.start.model !== model) ||
            child.inputDigest !== invocation.inputDigest)
            throw new Error('unobserved_child_invocation');
        childIds.add(child.invocationId);
        if (typeof child.transcript !== 'string' || await hash(child.transcript) !== child.sha256)
            throw new Error('invalid child transcript');
        const records = child.transcript.split('\n').filter(line => line.trim()).map(line => JSON.parse(line));
        if (records.length !== child.eventCount || records.some(item => !item || typeof item !== 'object' || Array.isArray(item)))
            throw new Error('invalid child transcript');
    }
    ...
    for (const [index, event] of journal.events.entries()) {
        ...
        if (event.type === 'invoke') {
            if (event.authenticatedContinuation && !continuation)
                throw new Error('unauthenticated review continuation');
            if (!parentEvents.some(item => item.type === 'item.completed' && item.item?.type === 'collab_tool_call' && item.item.tool === 'spawn_agent' && item.item.status === 'completed' && item.item.receiver_thread_ids?.includes(event.invocationId)))
                throw new Error('unobserved_child_invocation');
        }
        if (event.type === 'result') {
            const children = journal.children.filter(child => child.invocationId === event.invocationId);
            if (children.length !== 1 || observed.has(event.invocationId))
                throw new Error('unobserved_child_invocation');
            const child = children[0];
            const nativeRecords = child.transcript.split('\n').filter(line => line.trim()).map(line => JSON.parse(line));
            const nativeSession = nativeRecords.find(item => item.type === 'session_meta');
            const nativeMessage = nativeRecords.findLast(item => item.type === 'response_item' && item.payload?.type === 'message' && item.payload.role === 'assistant');
            const nativeText = nativeMessage?.payload.content?.filter(item => item.type === 'output_text').map(item => item.text).join('');
            if (nativeSession?.payload.id !== event.invocationId || nativeText !== child.finish?.last_assistant_message) {
                throw new Error('native child transcript differs from completion event');
            }
            if (child.parentAttemptId !== undefined && child.parentAttemptId !== attemptId)
                throw new Error('native child parent attempt mismatch');
            if (child.start?.hook_event_name !== 'SubagentStart' || child.finish?.hook_event_name !== 'SubagentStop' ||
                ...
                child.start.agent_type !== 'deos_reviewer' || child.finish.agent_type !== 'deos_reviewer')
                throw new Error('unobserved_child_invocation');
            ...
            if (!parentEvents.some(eventItem => eventItem.type === 'item.completed' && eventItem.item?.type === 'collab_tool_call' && eventItem.item.tool === 'spawn_agent' && eventItem.item.status === 'completed' && eventItem.item.receiver_thread_ids?.includes(event.invocationId)))
                throw new Error('unobserved_child_invocation');
            let last = child.finish.last_assistant_message;
            if (typeof last === 'string') {
                try {
                    last = JSON.parse(last.replace(/^```(?:json)?\s*\n|\n```$/g, ''));
                }
                catch (cause) {
                    throw new Error('invalid native child result', { cause });
                }
            }
            if (JSON.stringify(last) !== JSON.stringify(event.result))
                throw new Error('native child result differs from completion event');
            observed.add(event.invocationId);
        }
        state = reduceReviewCycle(state, event);
    }
    if (state.currentDigest !== journal.checkedCandidate.digest)
        throw new Error('checked candidate differs from review journal');
    return state;
}
